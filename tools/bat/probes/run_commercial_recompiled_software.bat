...
if "%~1"=="" (
  echo Uso: run_commercial_recompiled_software.bat "ruta\juego.cdi"
  echo.
  echo Genera/compila el ejecutable comercial y lo arranca desde el IP.BIN real.
  echo 0.0.170 A/B software: conserva PVR multihilo y desactiva D3D11 para comparar la misma escena.
...
echo [INFO] 0.0.170 SOFTWARE A/B: PVR MT activo, D3D11 desactivado; audio/timing identicos al runner normal.
echo [INFO] El runner normal NO activa --pvr-profile para evitar medir cada triangulo durante el juego.
echo [INFO] Revisa fps= y pvr-mt=; pvr-gpu debe aparecer off/wait/0/0/... y pvr-gpresent=off/...
