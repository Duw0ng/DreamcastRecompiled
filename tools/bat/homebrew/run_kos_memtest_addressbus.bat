...
set OUT=generated\kos_memtest_addressbus
set GENLOG=generated\kos_memtest_addressbus_codegen.txt
set RUNLOG=%OUT%\native_output.txt
...
echo =====================================================
echo  DreamcastRecomp 0.0.170 - KOS memTestAddressBus EXP
echo =====================================================
echo.
echo Funcion real: _memTestAddressBus
...
echo.
echo [1/4] Recompilando funcion real de KallistiOS
"%RECOMP%" "%ELF%" --function _memTestAddressBus --output "%OUT%" > "%GENLOG%" 2>&1
...
echo.
echo [4/4] Ejecutando con R4/R5 reales del ABI SH-4
...
echo.
echo [OK] _memTestAddressBus de KallistiOS se ejecuto nativamente y devolvio 0.
pause
exit /b 0

:unsupported
echo.
echo [INFO] Hay SH-4 aun no bajado a DCIR en tu memtest32.elf.
echo Comparte: %GENLOG%
pause
exit /b 4

:runtime_failure
echo.
echo [INFO] Se genero y compilo, pero fallo ejecutando.
echo Comparte %RUNLOG% y %GENLOG%
pause
exit /b 5

:wrong_result
echo.
echo [INFO] Termino, pero no con R0=0 / PC=0xFFFFFFFF.
echo Comparte %RUNLOG% y %GENLOG%
pause
exit /b 3
...
:error
echo.
echo [ERROR] Fallo el test KallistiOS memTestAddressBus.
pause
exit /b 1
