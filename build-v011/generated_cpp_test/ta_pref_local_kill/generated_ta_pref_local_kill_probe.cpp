#include "generated_ta_pref_local_kill_probe.hpp"
...
void recomp_00005000(SH4Context& ctx, DCRuntime& runtime) {
    bool delayed_t = false;
    std::uint32_t delayed_target = 0u;
    switch (ctx.pc) {
        case 0x00005000u: goto BB_5000;
        default: goto BB_5000;
    }
BB_5000:
    ctx.pc = 0x00005000u;
    DCR_SYNC_CURRENT_PC(runtime, ctx);
    if (dc_runtime_tick_fast(ctx, runtime, 3u)) return;
    // SH-4 @ 0x00005000u: STORE32
    dc_guest_write32_hot(runtime, ctx.r[4], ctx.r[2]);
    // SH-4 @ 0x00005002u: MOV_IMM
    ctx.r[4] = static_cast<std::uint32_t>(static_cast<std::int32_t>(0));
    // SH-4 @ 0x00005004u: PREF
    dc_guest_pref(runtime, ctx.r[4], 0x00005004u);

    return;
}

void register_generated_ta_pref_local_kill_probe(DCRuntime& runtime) {
    runtime.register_target(0x00005000u, &recomp_00005000);
}
