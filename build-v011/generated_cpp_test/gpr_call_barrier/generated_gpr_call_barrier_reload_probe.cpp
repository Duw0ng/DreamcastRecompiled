#include "generated_gpr_call_barrier_reload_probe.hpp"
...
void recomp_00008800(SH4Context& ctx, DCRuntime& runtime) {
    ...
    auto gpc_flush = [&](unsigned gpc_reason) {
        ...
        ctx.pr = gpc_pr;
        gpc_active = false;
        if (runtime.perf_profile_enabled) { runtime.gpr_cache_writebacks += 2u; if (gpc_reason == 1u) ++runtime.gpr_cache_tick_flushes; else if (gpc_reason == 2u) ++runtime.gpr_cache_barrier_flushes; else if (gpc_reason == 3u) ++runtime.gpr_cache_exit_flushes; }
    };
...
BB_8800:
    ...
    gpc_r2 += gpc_r3;
    gpc_flush(2u);
    // SH-4 @ 0x00008810u: CALL
    ctx.pr = 0x00008814u;
    call_recompiled(ctx, runtime, 0x00008B00u);
    if (runtime.sh4_async_redirect || ctx.pc != 0x00008814u) return;
    ctx.pc = 0x00008814u;
    gpc_reload();
    gpc_flush(3u);
    // SH-4 @ 0x00008812u: RETURN
    ctx.pc = ctx.pr;
    return;
...
void register_generated_gpr_call_barrier_reload_probe(DCRuntime& runtime) {
