#include "generated_ta_pref_cfg_kill_probe.hpp"
...
void recomp_00006000(SH4Context& ctx, DCRuntime& runtime) {
    bool delayed_t = false;
    std::uint32_t delayed_target = 0u;
    switch (ctx.pc) {
        case 0x00006000u: goto BB_6000;
        case 0x00006010u: goto BB_6010;
        default: goto BB_6000;
    }
BB_6000:
    ctx.pc = 0x00006000u;
    DCR_SYNC_CURRENT_PC(runtime, ctx);
    if (dc_runtime_tick_fast(ctx, runtime, 3u)) return;
    // SH-4 @ 0x00006000u: STORE32
    dc_guest_write32_hot(runtime, ctx.r[4], ctx.r[2]);
    // SH-4 @ 0x00006002u: BRANCH
    goto BB_6010;
...
BB_6010:
    ...
    // SH-4 @ 0x00006010u: MOV_IMM
    ctx.r[4] = static_cast<std::uint32_t>(static_cast<std::int32_t>(0));
    // SH-4 @ 0x00006012u: PREF
    dc_guest_pref(runtime, ctx.r[4], 0x00006012u);

    return;
}

void register_generated_ta_pref_cfg_kill_probe(DCRuntime& runtime) {
    runtime.register_target(0x00006000u, &recomp_00006000);
    runtime.register_target(0x00006010u, &recomp_00006000);
}
