#include "generated_fpscr_dn_sync_emit_probe.hpp"
...
void recomp_00004800(SH4Context& ctx, DCRuntime& runtime) {
    bool delayed_t = false;
    std::uint32_t delayed_target = 0u;
    switch (ctx.pc) {
        case 0x00004800u: goto BB_4800;
        default: goto BB_4800;
    }
BB_4800:
    ctx.pc = 0x00004800u;
    DCR_SYNC_CURRENT_PC(runtime, ctx);
    if (dc_runtime_tick_fast(ctx, runtime, 2u)) return;
    // SH-4 @ 0x00004800u: LDS_FPSCR
    dc_write_fpscr(ctx, ctx.r[2]);
    // SH-4 @ 0x00004802u: LDS_L_FPSCR
    { const std::uint32_t dc_tmp_fpscr = dc_guest_read32_hot(runtime, ctx.r[3]); dc_write_fpscr(ctx, dc_tmp_fpscr); }
    ctx.r[3] += 4u;

    return;
}

void register_generated_fpscr_dn_sync_emit_probe(DCRuntime& runtime) {
    runtime.register_target(0x00004800u, &recomp_00004800);
}
