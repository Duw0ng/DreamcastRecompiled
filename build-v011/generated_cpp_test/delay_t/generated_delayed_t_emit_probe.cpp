#include "generated_delayed_t_emit_probe.hpp"
...
void recomp_00003000(SH4Context& ctx, DCRuntime& runtime) {
    bool delayed_t = false;
    std::uint32_t delayed_target = 0u;
    switch (ctx.pc) {
        case 0x00003000u: goto BB_3000;
        default: goto BB_3000;
    }
BB_3000:
    ctx.pc = 0x00003000u;
    DCR_SYNC_CURRENT_PC(runtime, ctx);
    if (dc_runtime_tick_fast(ctx, runtime, 2u)) return;
    // SH-4 @ 0x00003000u: SAVE_T
    delayed_t = (ctx.sr & 1u) != 0u;
    // SH-4 @ 0x00003002u: CMP_EQ
    ctx.sr = (ctx.sr & ~1u) | ((ctx.r[4] == ctx.r[3]) ? 1u : 0u);
    // SH-4 @ 0x00003000u: BRANCH_IF_SAVED_TRUE
    if (delayed_t) { call_recompiled(ctx, runtime, 0x00003010u); return; }

    return;
}

void register_generated_delayed_t_emit_probe(DCRuntime& runtime) {
    runtime.register_target(0x00003000u, &recomp_00003000);
}
