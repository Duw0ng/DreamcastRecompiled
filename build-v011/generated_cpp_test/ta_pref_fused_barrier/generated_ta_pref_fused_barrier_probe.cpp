#include "generated_ta_pref_fused_barrier_probe.hpp"
...
BB_8000:
    ...
    if (dc_runtime_tick_fast(ctx, runtime, 10u)) return;
    // SH-4 @ 0x00008000u: STORE32_DISP
    dc_guest_write32_hot(runtime, ctx.r[4] + 0u, ctx.r[2]);
    // SH-4 @ 0x00008002u: STORE32_DISP
    dc_guest_write32_hot(runtime, ctx.r[4] + 4u, ctx.r[3]);
    // SH-4 @ 0x00008004u: STORE32_DISP
    dc_guest_write32_hot(runtime, ctx.r[4] + 8u, ctx.r[4]);
    // SH-4 @ 0x00008006u: STORE32_DISP
    dc_guest_write32_hot(runtime, ctx.r[4] + 12u, ctx.r[5]);
    // SH-4 @ 0x00008008u: STORE32_DISP
    dc_guest_write32_hot(runtime, ctx.r[4] + 16u, ctx.r[2]);
    // SH-4 @ 0x0000800Au: STORE32_DISP
    dc_guest_write32_hot(runtime, ctx.r[4] + 20u, ctx.r[3]);
    // SH-4 @ 0x0000800Cu: STORE32_DISP
    dc_guest_write32_hot(runtime, ctx.r[4] + 24u, ctx.r[4]);
    // SH-4 @ 0x0000800Eu: STORE32_DISP
    dc_guest_write32_hot(runtime, ctx.r[4] + 28u, ctx.r[5]);
    // SH-4 @ 0x0000800Fu: LOAD32
    ctx.r[7] = dc_guest_read32_hot(runtime, ctx.r[8]);
    // SH-4 @ 0x00008010u: PREF
    dc_pref_ta_native(runtime, ctx.r[4], 0x00008010u);

    return;
}

void register_generated_ta_pref_fused_barrier_probe(DCRuntime& runtime) {
