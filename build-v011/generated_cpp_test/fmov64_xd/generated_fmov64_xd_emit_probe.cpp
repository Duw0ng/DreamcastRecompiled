#include "generated_fmov64_xd_emit_probe.hpp"
...
void recomp_00004000(SH4Context& ctx, DCRuntime& runtime) {
    bool delayed_t = false;
    std::uint32_t delayed_target = 0u;
    switch (ctx.pc) {
        case 0x00004000u: goto BB_4000;
        default: goto BB_4000;
    }
BB_4000:
    ctx.pc = 0x00004000u;
    DCR_SYNC_CURRENT_PC(runtime, ctx);
    if (dc_runtime_tick_fast(ctx, runtime, 1u)) return;
    // SH-4 @ 0x00004000u: FMOV_LOAD_POSTINC
    if ((ctx.fpscr & (1u << 20)) == 0u) { dc_set_fr_bits(ctx, 1u, dc_guest_read32_hot(runtime, ctx.r[14])); ctx.r[14] += 4u; } else {
        dc_set_fmov64_bits(ctx, 1u, dc_guest_read64_fmov_hot(runtime, ctx.r[14])); ctx.r[14] += 8u; }

    return;
}

void register_generated_fmov64_xd_emit_probe(DCRuntime& runtime) {
    runtime.register_target(0x00004000u, &recomp_00004000);
}
