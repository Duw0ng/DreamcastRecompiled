# PVR / TA audit against Flycast - DreamcastRecomp 0.0.146

This audit records what is actually present in the 0.0.146 source tree. It deliberately distinguishes **decoded/retained state** from **fully rendered hardware semantics**. The reference was the current `flyinghead/flycast` PVR/TA source (`pvr_regs`, `ta`, `ta_vtx`, `ta_structs`, `TexCache` and D3D11 renderer/shaders).

| Area | 0.0.146 status | Notes |
|---|---|---|
...
| TA_LIST_INIT | Improved in 0.0.146 | Initializes guest TA pointer state. |
| TA_LIST_CONT | Improved in 0.0.146 | Continues same scene/render-pass without closing accumulated geometry. |
...
| CullMode | Implemented in D3D11 in 0.0.146 | none/front/back mapping follows Flycast D3D11 orientation. |
...
| IgnoreTexA | Implemented in D3D11 in 0.0.146 | Applied before ShadInstr. |
| ColorClamp | Implemented in D3D11 in 0.0.146 | Uses FOG_CLAMP_MIN/MAX. |
...
| SPG_STATUS | Improved in 0.0.146 | Programmed total lines, blank range and interlace field; still read-driven rather than cycle accurate. |
